...
提供统一运行时 facade，同时保留各平台的原生更新体验。它解决四件事：
...
3. **离线打包** —— wheel 内置目标平台的原生更新运行时，由支持的打包器直接收集，
   不访问网络。
4. **发布制作** —— 内置版本匹配的上游工具，用于生成密钥、签名更新包与制作
   macOS appcast。
...
平台 wheel 会携带对应的上游官方发布制作工具。wrapper 会透传所有参数，并保留
native tool 的退出码。
...
在 macOS 上生成或更新 appcast 与 delta updates：
...
macOS 命令调用 wheel 内置的 Sparkle universal2 `generate_keys`、
`sign_update`、`generate_appcast` 与 `BinaryDelta`。Windows x64 和 ARM64 的
`keys`、`sign` 命令调用官方 x64 `winsparkle-tool.exe`，其中 Windows ARM64
依赖操作系统的 x64 转译。Windows x86 不支持发布制作 CLI，Windows 也不支持
自动生成 appcast。这些限制只影响发布制作；三个 Windows wheel 架构仍都支持
WinSparkle 运行时更新。
